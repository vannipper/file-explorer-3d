...
class Player:
    ...
    def __init__(self, config=None, mouse_sensitivity=0.1, move_speed=5.0, fullscreen_toggle=None):
        ...
        # Vertical speed multiplier (makes Space/Shift feel faster)
        self.vertical_speed_mult = 1.5
    ...
    def update(self, dt):
        ...
        distance = self.camera.move_speed * dt
        
        # Apply horizontal movement
        if forward != 0 or right != 0:
            self.camera.move(forward * distance, right * distance)
            
        # Apply vertical movement with extra speed multiplier
        if up != 0:
            vertical_distance = distance * self.vertical_speed_mult
            self.camera.y += up * vertical_distance
